...
def train(args):
    ...
    logging.basicConfig(
        filename=configs["save_path"] + "/log_train.txt",
        level=logging.INFO,
    ...
    metric_values = []
    post_pred = Compose(
        [AsDiscrete(argmax=True, to_onehot=configs["classes_num"])]
    )
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if (epoch + 1) % 10 == 0 or (epoch + 1) == max_epochs:
                    ...
                    logging.info(
                        f"saved model at current epoch: {epoch + 1}, current mean dice: {metric:.4f}"
                    )
                    ...
                    torch.save(model.state_dict(), save_mode_path)
                    logging.info(
                        f"saved model at current epoch: {epoch + 1}, current best mean dice: {metric:.4f}"
                        f" at epoch: {best_metric_epoch}"
                    )

    train_time = time.time() - step_start
    logging.info(
        f"train completed in {train_time:.4f} seconds "
