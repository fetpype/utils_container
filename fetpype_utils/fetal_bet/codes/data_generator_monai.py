...
class CustomSuperpixelMask(Transform):
    def __init__(
        self, keys, n_segments, compactness, random_range=(-1, 1), p=0.5
    ):
        self.img_key = keys[0]
    ...
    def __call__(self, data):
        ...
        if mask.sum() > 500:
            ...
            intensity_option = np.random.choice(
                ["brighten", "blur", "noise", "darken"]
            )
            if intensity_option == "brighten":
                factor_map = np.random.uniform(
                    1.05,
                    1.2,
                    size=data[self.img_key][:, mask & segment_mask].shape,
                )
                factor_map = gaussian_filter(factor_map, sigma=0.4)
                data[self.img_key][:, mask & segment_mask] *= torch.from_numpy(
                    factor_map
                ).to(data[self.img_key].device)

            elif intensity_option == "blur":
                blurred_segment = gaussian_filter(
                    img.cpu().numpy(), sigma=generate_random_number(0.9, 1.0)
                )
                data[self.img_key][:, mask & segment_mask] = torch.from_numpy(
                    blurred_segment
                ).to(data[self.img_key].device)[mask & segment_mask]

            elif intensity_option == "noise":
                random_noise = torch.normal(
                    mean=0,
                    std=generate_random_number(0.2, 0.4),
                    size=((mask & segment_mask).sum().item(),),
                ).type(data[self.img_key].dtype)
        ...
        if self.config["augmentation"]:
            spatial_aug = tr.OneOf(
                [
                    ...
                    tr.RandRotate90d(
                        keys=["image", "label"], spatial_axes=(0, 1), prob=0.5
                    ),
                    tr.RandRotated(
                        keys=["image", "label"], range_x=(0.2, 1.0), prob=0.6
                    ),
                    tr.RandZoomd(
                        keys=["image", "label"],
                        min_zoom=0.8,
            ...
            intensity_aug = tr.OneOf(
                [
                    tr.RandGaussianNoised(
                        keys=["image"], mean=0, std=0.4, prob=0.5
                    ),
                    tr.RandBiasFieldd(
                        keys=["image"],
                        degree=4,
                        coeff_range=(0.05, 0.1),
                        prob=0.6,
                    ),
    ...
    def load_data(self):
        ...
        if self.config["validation_data_type"] == "path":
            val_images = sorted(
                glob(
                    os.path.join(
                        self.config["val_data_paths"], "images", "img_*.nii.gz"
                    )
                )
            )
            val_labels = sorted(
                glob(
                    os.path.join(
                        self.config["val_data_paths"], "masks", "mask_*.nii.gz"
                    )
                )
            )
        ...
        if self.config["fast_training"]:
            ...
            val_dataloader = ThreadDataLoader(
                val_dataset, num_workers=0, batch_size=1
            )
    ...
    def load_data(self):
        ...
        if self.config["test_data_type"] == "path":
            ...
            test_images = sorted(
                glob(
                    os.path.join(
                        self.config["test_data_paths"], "data", "**/*.nii.gz"
                    ),
                    recursive=True,
                )
            )
