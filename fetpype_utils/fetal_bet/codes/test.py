...
def test(args):
    ...
    model.load_state_dict(
        torch.load(configs["saved_model_path"], map_location=device)
    )
    ...
    test_dataloader, test_files, test_org_transforms_list = (
        fetal_test_data.load_data()
    )
    ...
    if configs["test_augmentation"]:
        ...
        with torch.no_grad():
            for file in test_files:
                ...
                for trans in spatial_transforms:
                    tta_transforms = tr.Compose(
                        test_org_transforms_list + trans
                    )
                    tta_post_transforms = tr.Compose(
                        [
                            tr.Activations(softmax=True),
                ...
                dice = dice_metric(
                    y_pred=tta_output_mean[None, ...],
                    y=unmodified_data["label"][None, None, ...],
                )
                iou = iou_metric(
                    y_pred=tta_output_mean[None, ...],
                    y=unmodified_data["label"][None, None, ...],
        ...
        with torch.no_grad():
            for i, test_data in enumerate(test_dataloader):
                test_inputs, test_labels = test_data["image"].to(
                    device
                ...
                test_time.append(
                    (time.time() - start_time) / test_inputs.shape[-1]
                )

                dice = dice_metric(
                    y_pred=from_engine(["pred"])(test_data), y=test_labels
                )
                iou = iou_metric(
                    y_pred=from_engine(["pred"])(test_data), y=test_labels
                )
                ...
                if (
                    configs["modality"] == "T2W"
                    or configs["modality"] == "otherscanners"
                ):
                    subject_id = os.path.basename(
                        os.path.dirname(test_inputs.meta["filename_or_obj"][0])
                    )
                ...
                if configs["plot_results"]:
                    if dice.item() < 0.85:
                        ...
                        original_image = tr.LoadImage()(
                            test_output[0].meta["filename_or_obj"]
                        )[0]
                        original_label = tr.LoadImage()(
                            test_labels[0].meta["filename_or_obj"]
                        )[0]
                        ...
                        new_filename = (
                            f"{parent_folder}_{filename_without_extension}.png"
                        )
                        save_dir = os.path.join(
                            configs["save_path"], "bad_dice_figs"
                        )
    ...
    if configs["save_metrics"]:
        ...
        dice_list = (
            dice_metric.get_buffer().detach().cpu().numpy()[:, 0]
        ).tolist()
        iou_list = (
            iou_metric.get_buffer().detach().cpu().numpy()[:, 0]
        ).tolist()
        modality = [configs["modality"]] * len(dice_list)
        method = [
            os.path.splitext(os.path.basename(configs["saved_model_path"]))[0]
        ] * len(dice_list)

        data = list(
            zip(method, modality, data_type, subj_list, dice_list, iou_list)
        )

        file_path = os.path.join(
            configs["save_path"],
            configs["modality"] + "_" + method[0] + ".csv",
        )
