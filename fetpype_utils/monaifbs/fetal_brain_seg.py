...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run fetal brain segmentation using MONAI DynUNet."
    )
    parser.add_argument(
        "--input_names",
        dest="input_names",
    ...
    assert len(args.input_names) == len(
        args.segment_output_names
    ), "The numbers of input output filenames do not match"
    ...
    for img, seg in zip(args.input_names, args.segment_output_names):
        ...
        out_filename = (
            img_filename + "_" + config["output"]["out_postfix"] + ".nii.gz"
            if flag_zip
        ...
        if not os.path.exists(out_filename):
            raise FileNotFoundError(
                "Network output file {} not found, "
                "check if the segmentation pipeline has failed".format(
                    out_filename
                )
            )
